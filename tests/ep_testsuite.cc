                  ...
                  std::string unit)
{
    ...
    printf("\n\n                                Percentile           \n");
    printf("  %-15s Mean     Median     95th     99th  Std Dev  Histogram of samples\n\n", "");
    // Finally, print out each set.
    for (const auto& stats : value_stats) {
        if (stats.median/1e6 < 1) {
            printf("%-15s %8.03f %8.03f %8.03f %8.03f %8.03f  ",
                    stats.name.c_str(), stats.mean/1e3, stats.median/1e3,
                    stats.pct95/1e3, stats.pct99/1e3, stats.stddev/1e3);
        } else {
            printf("%-8s (x1e3) %8.03f %8.03f %8.03f %8.03f %8.03f  ",
                    stats.name.c_str(), stats.mean/1e3, stats.median/1e6,
                    stats.pct95/1e6, stats.pct99/1e6, stats.stddev/1e6);
        }
...
static void perf_latency_core(ENGINE_HANDLE *h,
    ...
    hrtime_t stopped = gethrtime();
   // printf("started %lld\n", stopped-started);
       if((stopped - started) > 5599221387) {
        go =false;
       }
    }
        ...
        if (err == ENGINE_DISCONNECT) {
            ...
            switch (dcp_last_op) {
                ...
                case PROTOCOL_BINARY_CMD_DCP_DELETION:
                    ha->step_time.push_back(gethrtime() - step_start);
                    num_mutations++;
                    ha->timings.push_back(gethrtime());
                    ha->bytes_received.push_back(dcp_last_packet_size);//dcp_last_value.length());
...
static enum test_result perf_dcp_latency_and_bandwidth(ENGINE_HANDLE *h,
    ...
    std::vector<std::pair<std::string, std::vector<hrtime_t>*> > all_timings, all_steps;
    ...
    struct Handle_args ha1(h, h1, item_count, typeOfData, "VB0",
                           0xFFFFFF00, 0, false);
    ...
    struct Handle_args ha2(h, h1, item_count, typeOfData, "VB1",
                           0xFF000000, 1, false);
    ...
    for (size_t i = 0; i < iterations.size(); ++i) {
        ...
        iterations[i].step_time = dcp_ha.step_time;
        ...
        all_steps.push_back(std::make_pair(dcp_ha.name, &iterations[i].step_time));
    }

    int printed = 0;
    printf("\n\n=== %s Latency (front to back) - %zu items (µs) %n",
    ...
    printed = 0;
    printf("=== %s Latency (just step) - %zu items (µs) %n",
           title.c_str(), item_count, &printed);
    fillLineWith('=', 86-printed);

    print_values(all_steps, "µs");
...
static enum test_result perf_dcp_latency_with_padded_json(ENGINE_HANDLE *h,
    ...
    return perf_dcp_latency_and_bandwidth(h, h1,
                            "DCP In-memory (JSON-PADDED) [VB0 vs. VB1]",
...
static enum test_result perf_dcp_latency_with_random_json(ENGINE_HANDLE *h,
                                                          ENGINE_HANDLE_V1 *h1) {
    return perf_dcp_latency_and_bandwidth(h, h1,
                            "DCP In-memory (JSON-RAND) [VB0 vs. VB1]",
...
static enum test_result perf_dcp_latency_with_random_binary(ENGINE_HANDLE *h,
                                                            ENGINE_HANDLE_V1 *h1) {
    return perf_dcp_latency_and_bandwidth(h, h1,
                            "DCP In-memory (BINARY-RAND) [VB0 vs. VB1]",
...
engine_test_t* get_tests(void) {
    ...
    TestCase tc[] = {
        ...
        TestCase("DCP impact on front-end latency", perf_latency_dcp_impact,
                ...
                TestCase("DCP impact on front-end padded json ", perf_dcp_latency_with_padded_json,
                 test_setup, teardown,
                 "ht_size=393209",
                 prepare, cleanup),

                TestCase("DCP impact on front-end random json ", perf_dcp_latency_with_random_json,
                 test_setup, teardown,
                 "ht_size=393209",
                 prepare, cleanup),

                                TestCase("DCP impact on front-end random binary ", perf_dcp_latency_with_random_binary,
                 test_setup, teardown,
                 "ht_size=393209",
                 prepare, cleanup),
