...
static enum test_result test_dcp_early_termination(ENGINE_HANDLE* h,
    ...
    for (int i = 0; i < streams; i++) {
        ...
        check(set_vbucket_state(h, h1, i, vbucket_state_active),
            "Failed to set vbucket state");
        std::string statkey;
        statkey = "vb_" + std::to_string(i) + ":0:id";
        vbuuid[i] = get_ull_stat(h, h1, statkey.c_str(), "failovers");

        /* Set n items */

        for (int count = 0; count < num_items; count++) {
            std::string key;
            key = "KEY" + std::to_string(i) + std::to_string(count);
            check(ENGINE_SUCCESS ==
    ...
    const void *cookie = testHarness.create_cookie();
    uint32_t opaque = 1;
    check(h1->dcp.open(h, cookie, ++opaque, 0, DCP_OPEN_PRODUCER, (void*)"unittest",
                       strlen("unittest")) == ENGINE_SUCCESS,
          "Failed dcp producer open connection.");
    ...
    // let all tasks run
    wait_for_stat_to_be(h, h1, "ep_workload:ready_tasks", 0, "workload");
    return SUCCESS;
}
