...
TEST(ManifestTest, validation) {
    ...
    for (auto& manifest : invalidManifests) {
        EXPECT_THROW(Collections::Manifest m(manifest.c_str(), manifest.size()),
                     std::invalid_argument)
        << "Didn't throw exception for an invalid manifest " + manifest;
    }

    for (auto& manifest : validManifests) {
        EXPECT_NO_THROW(Collections::Manifest m(manifest.c_str(), manifest.size()))
...
TEST(ManifestTest, getRevision) {
    ...
    for (auto& manifest : validManifests) {
        Collections::Manifest m(manifest.second.c_str(),
                                manifest.second.size());
