...
static void launch_set_thread(void *arg) {
    ...
    for (i = 0; i < NUM_ITEMS; ++i) {
        ...
        key << "key-" << i;
        queued_item qi(new Item(ItemKey(key.str().c_str(), key.str().length(), 0), args->vbucket->getId(),
                                queue_op_set, 0, 0));
...
void basic_chk_test() {
    ...
    RCPtr<VBucket> vbucket(new VBucket(0, vbucket_state_active, global_stats,
                                       checkpoint_config, NULL, storagePool, 0, 0, 0, NULL));
    ...
    *counter = 0;
    ...
    queued_item qi(new Item(ItemKey(key.c_str(), key.length(), 0), vbucket->getId(), queue_op_flush, 0xffff, 0));
