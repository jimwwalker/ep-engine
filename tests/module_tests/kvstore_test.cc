...
TEST_F(CouchKVStoreMetaData, basic) {
    ...
    EXPECT_EQ(16, MetaData::getMetaDataSize(MetaData::V0));
    EXPECT_EQ(16 + 2, MetaData::getMetaDataSize(MetaData::V1));
    EXPECT_EQ(16 + 2 + 1, MetaData::getMetaDataSize(MetaData::V2));
}
...
TEST_F(CouchKVStoreMetaData, overlay) {
    ...
    EXPECT_EQ(MetaData::V0, metadata->getVersion());

    data.resize(16 + 2);
    meta.buf = data.data();
    meta.size = data.size();
    metadata = MetaDataFactory::createMetaData(meta);
    EXPECT_EQ(MetaData::V1, metadata->getVersion());

    data.resize(16 + 2 +1);
    meta.buf = data.data();
    meta.size = data.size();
    metadata = MetaDataFactory::createMetaData(meta);
    EXPECT_EQ(MetaData::V2, metadata->getVersion());
...
TEST_F(CouchKVStoreMetaData, overlayExpands1) {
    ...
    EXPECT_EQ(out.size, MetaData::getMetaDataSize(MetaData::V2));
...
TEST_F(CouchKVStoreMetaData, writeToOverlay) {
    ...
    EXPECT_EQ(MetaData::V2, metadata->getVersion()); // Is it V2?
