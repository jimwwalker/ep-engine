...
public:
    WarmupInitialize(EventuallyPersistentStore &st,
                     Warmup *w) :
        GlobalTask(&st.getEPEngine(), MY_TASK_ID(WarmupInitialize), 0, false),
        _warmup(w) {
...
public:
    WarmupCreateVBuckets(EventuallyPersistentStore &st,
        ...
        GlobalTask(&st.getEPEngine(), MY_TASK_ID(WarmupCreateVBuckets), 0, false),
        _shardId(sh),
        _warmup(w) {
        _warmup->addToTaskSet(uid);
    }
...
public:
    WarmupEstimateDatabaseItemCount(EventuallyPersistentStore &st,
                                    uint16_t sh, Warmup *w):
        GlobalTask(&st.getEPEngine(), MY_TASK_ID(WarmupEstimateDatabaseItemCount), 0, false),
        _shardId(sh),
        _warmup(w) {
        _warmup->addToTaskSet(uid);
    }
...
public:
    WarmupKeyDump(EventuallyPersistentStore &st,
                  uint16_t sh, Warmup *w) :
        GlobalTask(&st.getEPEngine(), MY_TASK_ID(WarmupKeyDump), 0, false),
        _shardId(sh),
        _warmup(w) {
        _warmup->addToTaskSet(uid);
    }
...
public:
    WarmupCheckforAccessLog(EventuallyPersistentStore &st,
                            Warmup *w) :
        GlobalTask(&st.getEPEngine(), MY_TASK_ID(WarmupCheckforAccessLog), 0, false),
        _warmup(w) {
        _warmup->addToTaskSet(uid);
    }
...
public:
    WarmupLoadAccessLog(EventuallyPersistentStore &st,
                        uint16_t sh, Warmup *w) :
        GlobalTask(&st.getEPEngine(), MY_TASK_ID(WarmupLoadAccessLog), 0, false),
        _shardId(sh),
        _warmup(w) {
        _warmup->addToTaskSet(uid);
    }
...
public:
    WarmupLoadingKVPairs(EventuallyPersistentStore &st,
                         uint16_t sh, Warmup *w) :
        GlobalTask(&st.getEPEngine(), MY_TASK_ID(WarmupLoadingKVPairs), 0, false),
        _shardId(sh),
        _warmup(w) {
        _warmup->addToTaskSet(uid);
    }
...
public:
    WarmupLoadingData(EventuallyPersistentStore &st,
                      uint16_t sh, Warmup *w) :
        GlobalTask(&st.getEPEngine(), MY_TASK_ID(WarmupLoadingData), 0, false),
        _shardId(sh),
        _warmup(w) {
        _warmup->addToTaskSet(uid);
    }
...
public:
    WarmupCompletion(EventuallyPersistentStore &st,
                     Warmup *w) :
        GlobalTask(&st.getEPEngine(), MY_TASK_ID(WarmupCompletion), 0, false),
        _warmup(w) {
        _warmup->addToTaskSet(uid);
    }
