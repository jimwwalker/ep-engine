 ...
 */
class BloomFilterCallback : public Callback<ItemKey&, bool&> {
public:
    ...
    void callback(ItemKey& key, bool& isDeleted) {
        cb_assert(store);
...
uint16_t EventuallyPersistentStore::getCommitInterval(uint16_t shardId) {
    return vbMap.shards[shardId]->getCommitInterval();
}

uint16_t EventuallyPersistentStore::decrCommitInterval(uint16_t shardId) {
    printf("SHARDID %d\n", shardId);
    return vbMap.shards[shardId]->decrCommitInterval();
}
...
ENGINE_ERROR_CODE EventuallyPersistentStore::setVBucketState(uint16_t vbid,
    ...
    if (vb) {
        ...
        HashTable& hashTable = engine.getStoragePool().createHashTable(engine.getBucketId(), vbid, engine.getEpStats());
    ...
    if (vb) {
        ...
        if (config.isBfilterEnabled()) {
            ...
            if (eviction_policy == VALUE_ONLY) {
                ...
                shared_ptr<Callback<ItemKey&, bool&> >
        ...
        shared_ptr<Callback<ItemKey&, uint64_t&> >
...
void EventuallyPersistentStore::bgFetch(const ItemKey &key,
    ...
    if (multiBGFetchEnabled()) {
        ...
        VBucketBGFetchItem * fetchThis = new VBucketBGFetchItem(cookie,
                                                                isMeta);
        vb->queueBGFetchItem(key, fetchThis);
    ...
    void callback(mutation_result &value) {
        if (value.first == 1) {
            ...
            if (value.first == 0) {
                ...
                if (v) {
                    std::stringstream ss;
                    ss << "Persisting ``" << queuedItem->getRawKey() << "'' on vb"
        ...
        if (!items.empty()) {
            ...
            for(; it != items.end(); ++it) {
                if ((*it)->getOperation() != queue_op_set &&
                    ...
                    continue;
                } else if (!prev || prev->getItemKey() != (*it)->getItemKey()) {
                    prev = (*it).get();
                    ++items_flushed;
