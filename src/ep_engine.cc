...
public:
    StatCheckpointTask(EventuallyPersistentEngine *e, const void *c,
            ADD_STAT a) : GlobalTask(e, MY_TASK_ID(StatCheckpointTask),
                                     0, false),
...
public:
    FetchAllKeysTask(EventuallyPersistentEngine *e, const void *c,
        ...
        GlobalTask(e, MY_TASK_ID(FetchAllKeysTask), 0, false), engine(e), cookie(c),
...
void EpEngineTaskable::logQTime(TaskId taskType, hrtime_t enqTime) {
    myEngine->getEpStore()->logQTime(taskType, enqTime);
}

void EpEngineTaskable::logRunTime(TaskId taskType, hrtime_t runTime) {
