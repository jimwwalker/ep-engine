...
extern "C" {
    ...
    static protocol_binary_response_status evictKey(
        ...
        ItemKey key(keyz, keylen, e->getBucketId());
    ...
    static ENGINE_ERROR_CODE getLocked(EventuallyPersistentEngine *e,
        ...
        *res = PROTOCOL_BINARY_RESPONSE_SUCCESS;
        ...
        ItemKey key(keyp, ntohs(req->request.keylen), e->getBucketId());
    ...
    static ENGINE_ERROR_CODE getReplicaCmd(EventuallyPersistentEngine *e,
        ...
        ItemKey itemKey(((char *)request) + sizeof(req->message.header),
                        keylen,
                        e->getBucketId());
                                       ...
                                       ADD_RESPONSE response)
    {
        ...
        if (itm && request->request.opcode == PROTOCOL_BINARY_CMD_GET_LOCKED) {
            ...
            uint32_t flags = itm->getFlags();
            rv = sendResponse(response, static_cast<const void *>(itm->getRawKey()),
                              itm->getNKey(),
                                            ...
                                            uint8_t nru)
    {
        ...
        ENGINE_ERROR_CODE errCode = ENGINE_DISCONNECT;
        ConnHandler* conn = getHandle(handle)->getConnHandler(cookie);
        ItemKey itemKey(static_cast<const char*>(key),
                        nkey,
                        getHandle(handle)->getBucketId());
        if (conn) {
            errCode = conn->mutation(opaque, itemKey, value, nvalue, cas,
                                            ...
                                            uint16_t nmeta)
    {
        ...
        ItemKey itemKey(static_cast<const char*>(key),
                        ...
                        getHandle(handle)->getBucketId());
        if (conn) {
            errCode = conn->deletion(opaque, itemKey, cas, vbucket, bySeqno,
                                     revSeqno, meta, nmeta);
        }
        releaseHandle(handle);
        return errCode;
                                              ...
                                              uint16_t nmeta)
    {
        ...
        ItemKey itemKey(static_cast<const char*>(key),
                        ...
                        getHandle(handle)->getBucketId());
        if (conn) {
            errCode = conn->expiration(opaque, itemKey, cas, vbucket, bySeqno,
                                       revSeqno, meta, nmeta);
        }
        releaseHandle(handle);
        return errCode;
    ...
    taskable(this)
{
    ...
    static bucket_id_t staticBucketID = 0x0acecafe;
...
ENGINE_ERROR_CODE EventuallyPersistentEngine::store(const void *cookie,
    ...
    case OPERATION_PREPEND: {
        bool locked;
        do {
            if ((ret = get(cookie, &i, it->getRawKey(),
                                                        ...
                                                        uint16_t vbucket)
{
    ...
    case TAP_ACK:
        ret = processTapAck(cookie, tap_seqno, tap_flags,
                            std::string(static_cast<const char*>(key), nkey));
        break;
    case TAP_FLUSH:
        ret = flush(cookie, 0);
    ...
    case TAP_DELETION:
        {
            ...
            ret = connection->deletion(0, k, cas, vbucket, 0, revSeqno,
                                       NULL, 0);
        }
        break;
    ...
    case TAP_MUTATION:
        {
            ...
            ret = connection->mutation(0, k, data, ndata, cas, vbucket,
...
ENGINE_ERROR_CODE EventuallyPersistentEngine::getStats(const void* cookie,
    ...
    if (stat_key == NULL) {
        ...
        ItemKey itemKey(key.c_str(), key.length(), getBucketId());
        uint16_t vbucket_id(0);
...
ENGINE_ERROR_CODE EventuallyPersistentEngine::observe(
    ...
    while (offset < data_len) {
        ...
        ItemKey key(data + offset, keylen, getBucketId());
        offset += keylen;
                                                      ...
                                                      ADD_RESPONSE response)
{
    ...
    ItemKey key((char *)(request->bytes + sizeof(request->bytes) + extlen),
                                ...
                                ADD_RESPONSE response)
{
    ...
    *(ext_meta) = datatype;
    Item *itm = new Item(ItemKey(reinterpret_cast<const char*>(key), keylen, getBucketId()),
...
ENGINE_ERROR_CODE EventuallyPersistentEngine::deleteWithMeta(
    ...
    ItemKey key(key_ptr, nkey, getBucketId());
...
EventuallyPersistentEngine::returnMeta(const void* cookie,
    ...
    if (mutate_type == SET_RET_META || mutate_type == ADD_RET_META) {
        ...
        uint8_t ext_len = EXT_META_LEN;
        *(ext_meta) = datatype;
        ItemKey itemkey(reinterpret_cast<const char*>(key), keylen, getBucketId());
        Item *itm = new Item(itemkey, flags, exp, dta, vallen, ext_meta,
        ...
        ItemKey itemkey(reinterpret_cast<const char*>(key), keylen, getBucketId());
        ret = epstore->deleteItem(itemkey, &cas, vbucket, cookie, false,
                                  &itm_meta, &mut_info);
        if (ret == ENGINE_SUCCESS) {
...
ENGINE_ERROR_CODE EventuallyPersistentEngine::getRandomKey(const void *cookie,
    ...
    if (ret == ENGINE_SUCCESS) {
        Item *it = gv.getValue();
        uint32_t flags = it->getFlags();
        ret = sendResponse(response, static_cast<const void *>(it->getRawKey()),
                           it->getNKey(),
                           (const void *)&flags, sizeof(uint32_t),
