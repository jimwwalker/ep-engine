...
Manifest::Manifest()
     :  revision(0),
        separator(":"),
        containsDefaultCollection(true) {
    collections.insert(defaultCollection);
}

Manifest::Manifest(const char* json, size_t jsonLen) {
    unique_cJSON_ptr cjson(cJSON_Parse(json));
    if (!cjson) {
        throw std::invalid_argument("Manifest::Manifest cannot parse json");
    }

    auto jsonRevision = cJSON_GetObjectItem(cjson.get(), "revision");
    if (!jsonRevision || jsonRevision->type != cJSON_Number) {
        throw std::invalid_argument("Manifest::Manifest cannot find valid "
                                    "revision: " +
    ...
    if (!jsonCollections || jsonCollections->type != cJSON_Array) {
        ...
        for (int ii = 0; ii < cJSON_GetArraySize(jsonCollections); ii++) {
            ...
            if (!collection || collection->type != cJSON_String) {
                throw std::invalid_argument("Manifest::Manifest cannot find "
                                            "valid collection for index " +
                                            std::to_string(ii) +
                                            (!collection ? " nullptr"
                                                : std::to_string(collection->type)));
            } else {
                containsDefaultCollection = containsDefaultCollection ||
                    (defaultCollection.compare(collection->valuestring) == 0);
                collections.insert(collection->valuestring);
            }
        }
    }
}
...
const std::string Manifest::defaultCollection("$default");
};