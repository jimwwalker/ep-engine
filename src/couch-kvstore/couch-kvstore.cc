...
static int edit_docinfo_hook(DocInfo **info, const sized_buf *item) {
    ...
    if (documentMetaData->getVersion() == MetaData::Version::V0) {
    ...
    } else if (documentMetaData->getVersion() == MetaData::Version::V1) {
    ...
    *docInfo = **info;
    ...
    // Move the metadata from MetaData to docInfo.
    ...
    if (metaOnly || (fetchDelete && docinfo->deleted)) {
        ...
        if (errCode == COUCHSTORE_SUCCESS) {
            if (docinfo->deleted) {
                 ...
                 */
                uint8_t extMeta = 0;
                if (metadata->getVersion() == MetaData::Version::V0) {
