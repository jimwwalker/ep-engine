...
CouchRequest::CouchRequest(const Item &it, uint64_t rev,
      ...
      : IORequest(it.getVBucketId(), cb, del, it.getKey()), value(it.getValue()),
        meta(MetaDataFactory::createMetaData()),
        fileRevNum(rev) {
    ...
    // Now move to Couchstore::docInfo's sized_buf
    meta->moveToSizedBuf(dbDocInfo.rev_meta);
...
static int edit_docinfo_hook(DocInfo **info, const sized_buf *item) {
    ...
    if (documentMetaData->getVersion() == MetaData::V0) {
    ...
    } else if (documentMetaData->getVersion() == MetaData::V1) {
    ...
    *docInfo = **info;
    ...
    // Move the metdata from MetaData to docInfo.
    metadata->moveToSizedBuf(docInfo->rev_meta);
...
static int time_purge_hook(Db* d, DocInfo* info, void* ctx_p) {
    ...
    if (info->rev_meta.size >= MetaData::getMetaDataSize(MetaData::V0)) {
    ...
    if (metaOnly || (fetchDelete && docinfo->deleted)) {
        ...
        if (errCode == COUCHSTORE_SUCCESS) {
            if (docinfo->deleted) {
                 ...
                 */
                uint8_t extMeta = 0;
                if (metadata->getVersion() == MetaData::V0) {
...
int CouchKVStore::recordDbDump(Db *db, DocInfo *docinfo, void *ctx) {
    ...
    if (sctx->valFilter != ValueFilter::KEYS_ONLY && !docinfo->deleted) {
         ...
         */
        if (docinfo->rev_meta.size == metadata->getMetaDataSize(MetaData::V0) ||
