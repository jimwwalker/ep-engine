...
struct AllKeysCtx {
    AllKeysCtx(shared_ptr<Callback<uint16_t&, char*&> > callback, uint32_t cnt)
...
CouchRequest::CouchRequest(const Item &it, uint64_t rev,
    ...
    : IORequest(it.getVBucketId(), cb, del, it.getRawKey()), value(it.getValue()),
      fileRevNum(rev)
{
...
CouchKVStore::CouchKVStore(KVStoreConfig &config,
    ...
    intransaction(false), backfillCounter(0), bucketId(bId)
{
...
CouchKVStore::CouchKVStore(const CouchKVStore &copyFrom) :
    ...
    intransaction(false), bucketId(copyFrom.bucketId)
{
    createDataDir(dbname);
    statCollectingFileOps = getCouchstoreStatsOps(&st.fsStats);
}
...
void CouchKVStore::get(const ItemKey &key, uint16_t vb,
...
void CouchKVStore::getWithHeader(void *dbHandle, const ItemKey &key,
    ...
    GetValue rv;
    couchstore_error_t errCode = couchstore_docinfo_by_id(db,
                                                          key.getKey(),
                                                          key.getKeyLen(),
                                                          &docInfo);
    ...
    if (errCode != COUCHSTORE_SUCCESS) {
        if (!getMetaOnly) {
            ...
            LOG(EXTENSION_LOG_WARNING,
                ...
                vb, key.getKey(), couchstore_strerror(errCode),
        ...
        if (errCode == COUCHSTORE_SUCCESS) {
            st.readSizeHisto.add(key.getKeyLen() + rv.getValue()->getNBytes());
        }
    }
...
void CouchKVStore::getMulti(uint16_t vb, vb_bgfetch_queue_t &itms) {
    ...
    vb_bgfetch_queue_t::iterator itr = itms.begin();
    for (; itr != itms.end(); ++itr) {
        ids[idx].size = itr->first.getKeyLen();
        ids[idx].buf = const_cast<char *>(itr->first.getKey());
        ++idx;
    }
...
static int time_purge_hook(Db* d, DocInfo* info, void* ctx_p) {
    ...
    if (info->rev_meta.size >= DEFAULT_META_LEN) {
        ...
        if (info->deleted) {
            ...
            ItemKey key((const char *)info->id.buf, info->id.size, ctx->bucketId);
...
bool CouchKVStore::compactVBucket(const uint16_t vbid,
    ...
    // couchstore callback needs to pass bucketId, use the ctx for this
    hook_ctx->bucketId = bucketId;
...
scan_error_t CouchKVStore::scan(ScanContext* ctx) {
    ...
    // recordDbDumpC is static so can't access our bucketId, so add to ctx
    ...
    if (metaOnly || (fetchDelete && docinfo->deleted)) {
        ...
        if (errCode == COUCHSTORE_SUCCESS) {
            if (docinfo->deleted) {
                ...
                Item *it = new Item(ItemKey(docinfo->id.buf,
                                            (size_t)docinfo->id.size,
                                            bucketId),
                                    itemFlags, (time_t)exptime,
                                    valuePtr, valuelen,
                                    ext_meta, ext_len,
                                    cas, docinfo->db_seq,
                                    vbId, docinfo->rev_seq);
...
int CouchKVStore::recordDbDump(Db *db, DocInfo *docinfo, void *ctx) {
    ...
    ItemKey docKey(docinfo->id.buf, docinfo->id.size, sctx->bucketId);
...
void CouchKVStore::readVBState(Db *db, uint16_t vbId) {
    ...
    if (errCode == COUCHSTORE_SUCCESS) {
        ...
        LOG(EXTENSION_LOG_WARNING,
            "Failed to read database info for vBucket = %d", vbId);
        abort();
    }
    ...
    if (errCode != COUCHSTORE_SUCCESS) {
        LOG(EXTENSION_LOG_DEBUG,
            "Failed to retrieve stat info for vBucket=%d error=%s [%s]",
...
int CouchKVStore::getMultiCb(Db *db, DocInfo *docinfo, void *ctx) {
    ...
    ItemKey itemKey(docinfo->id.buf, docinfo->id.size, cbCtx->cks.getBucketId());


    vb_bgfetch_queue_t::iterator qitr = cbCtx->fetches.find(itemKey);
    ...
    if (errCode != COUCHSTORE_SUCCESS && !meta_only) {
        LOG(EXTENSION_LOG_WARNING, "Failed to fetch data from database, "
            ...
            itemKey.getKey(), couchstore_strerror(errCode),
            couchkvstore_strerrno(db, errCode).c_str());
        st.numGetFailure++;
    }
