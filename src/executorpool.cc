...
void ExecutorPool::_registerTaskable(Taskable *taskable) {
    ...
    if (priority < HIGH_BUCKET_PRIORITY) {
        ...
        queueName = "LowPrioQ_";
        LOG(EXTENSION_LOG_WARNING, "Taskable %s registered with low priority",
        ...
        queueName = "HiPrioQ_";
        LOG(EXTENSION_LOG_WARNING, "Taskable %s registered with high priority",
            taskable->getName().c_str());
    }
...
bool ExecutorPool::_stopTaskGroup(uintptr_t taskGID,
...
bool ExecutorPool::stopTaskGroup(uintptr_t taskGID,
                                 task_type_t taskType) {
    EventuallyPersistentEngine *epe = ObjectRegistry::onSwitchThread(NULL, true);
