    ...
    lowPrioritySnapshot(false), highPriorityCount(0)
{
    ...
    rwUnderlying = KVStoreFactory::create(config, store.getEPEngine().getBucketId(), false);
    roUnderlying = KVStoreFactory::create(config, store.getEPEngine().getBucketId(), true);
