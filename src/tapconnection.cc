...
ENGINE_ERROR_CODE ConnHandler::mutation(uint32_t opaque, const ItemKey& key, const void* value,
...
ENGINE_ERROR_CODE ConnHandler::deletion(uint32_t opaque, const ItemKey& key, uint64_t cas,
...
ENGINE_ERROR_CODE ConnHandler::expiration(uint32_t opaque, const ItemKey& key, uint64_t cas,
                                          uint16_t vbucket, uint64_t bySeqno,
                                          uint64_t revSeqno, const void* meta,
                                          uint16_t nmeta) {
    LOG(EXTENSION_LOG_WARNING, "%s Disconnecting - This connection doesn't "
...
Item* TapProducer::getNextItem(const void *c, uint16_t *vbucket, uint16_t &ret,
    ...
    if (checkpoint_msg.get() != NULL) {
        ...
        *vbucket = checkpoint_msg->getVBucketId();
        ...
        itm = new Item(ItemKey(key.data(), key.length()), /*flags*/0, /*exp*/0,
