...
StoragePoolFetcher::StoragePoolFetcher(StoragePool& sp, StoragePoolShardTaskable& spt)
    : storagePool(sp), taskable(spt) {
}

void StoragePoolFetcher::start() {
    ExTask task = new StoragePoolFetcherTask(&taskable, *this, Priority::BgFetcherPriority, 0.0, false);
    taskId = task->getId();
...
size_t StoragePoolFetcher::doFetch(EventuallyPersistentEngine* currentEngine, uint16_t vb, vb_bgfetch_queue_t& items) {
    ...
    if (totalfetches > 0) {
        currentEngine->getEpStore()->completeBGFetchMulti(vb, fetchedItems, startTime);
        currentEngine->getEpStats().getMultiHisto.add((gethrtime()-startTime)/1000, totalfetches);
    }
    ...
    for (auto itemsItr : items) {
        ...
        itemsItr.second.front()->delValue();
        for (auto item : itemsItr.second) {
            delete item;
        }
    }
    ...
    while (pendingBucketIter != pending.end()) {
        ...
        while (iter != pendingBucketIter->second.end()) {
            ...
            if ((currentEngine = storagePool.getEngine(pendingBucketIter->first)) != nullptr) {
                RCPtr<VBucket> vb = currentEngine->getEpStore()->getVBucket(vbid);
                vb_bgfetch_queue_t itemsForFetching;
                if (vb && vb->getBGFetchItems(itemsForFetching)) {
                    // set the engine for memory tracking
                    EventuallyPersistentEngine *epe = ObjectRegistry::onSwitchThread(currentEngine, true);
                    totalFetched += doFetch(currentEngine, vbid, itemsForFetching);
                    ObjectRegistry::onSwitchThread(epe, false);
                }

            } else {
                // the engine has gone, empty the set and move onto the next bucket
                lockHolder.lock();
                pendingBucketIter->second.clear();
                break; // leaving the while (iter != pendingBucketIter->second.end()) loop
            }
        ...
        // else it's not empty, means a bucket/VB became again, don't delete and try this bucket again
...
bool StoragePoolFetcher::run() {
    // lock and see if the pending map is now empty, buckets may of become pending again...
    LockHolder lockHolder(pendingMutex);
