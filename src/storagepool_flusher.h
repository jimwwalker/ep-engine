...
public:
    ...
    */
    ...
    /*
        ...
        Returns the seconds the task should sleep for and runAgain is set to also indicate
        if the task is to be re-scheduled.
    */
...
private:
    ...
    enum State {
        ...
        FLUSH,
        // flush vbuckets as they become ready *and* force a flush on specific buckets and notify a waiter
        FLUSH_AND_NOTIFY_FOR_ENGINE,
        // flusher is shutting down, empty queues and stop
        SHUTDOWN // TYNSET: shutdown not currently used and do we need shutdown force?
    };
    ...
    */
    ...
    /*
        ...
        2. check if a bucket is moving between pause/resume and manage the pendingPaused data
    */
    ...
    */
    void flushOneBucket(EventuallyPersistentEngine* engine, std::set<uint16_t>& vbuckets, LockHolder& lockHolder);

    /*
        ontask
        Perform any logic required to allow the flusher to correctly wakeup/timeout
        any callers using the checkpoint/seqno notification commands.
    */
    void setupCheckpointPending(EventuallyPersistentEngine* engine, uint16_t vbid);
    ...
    */
    ...
    /*
        This mutex currently provides serial access to all maps and engineQueue.
    */
    Mutex pendingMutex;
    ...
    */
    ...
    /*
        A list of SyncObjects that are blocking - part of flushEngineAndWait.
    */
    std::vector<std::pair<SyncObject*, EventuallyPersistentEngine*> > engineQueue;
